...
Desde o *R* as primeiras linhas desse arquivo podem ser lidas com o comando em baixo:
...
No trabalho com distribuições Linux devemos sempre preferir versões com soporte estável, nesse momento as versões de Soporte por Longo Tempo (LTS) são as 14.04 (abril de 2014, codename `trusty`) e a 15.04 (abril de 2015, codename `vivid`), você pode saber mais sobre as [versões de Ubuntu](http://releases.ubuntu.com/). Para saber qual é a versão do sistema operacional (SO) de sua PC digite na terminal linux o seguinte comando:
...
# Pacotes recomendados para apliacação em Meteorologia
...
A continuação mostra-se linhas a executar desde a console de *R*, esses comandos instalarám alguns dos pacotes e librerias de linux que são úteis na meteorologia e outras utilidades. 
...
system("sudo apt-get install g++ gcc-4.8-doc libstdc++6-4.8-dbg lib32stdc++6-4.8-dbg libx32stdc++6-4.8-dbg libstdc++5") 
## Deve pedir uma senha de autorização de super usuário
system("sudo apt-get install build-essential lib32z1 lib32ncurses5 lib32bz2-1.0 libunwind8 gfortran htop")
system("sudo apt-get install netcdf-bin proj-bin libproj-dev gdal-bin")
system("sudo apt-get install libwxgtk-media3.0-0 libxml2-dev libcurl4-gnutls-dev libpq-dev postgresql-9.3") 
system("sudo apt-get install r-cran-xml r-cran-rcurl") 
system("sudo apt-get install libcurl4-openssl-dev libboost-dev") 
system("sudo apt-get install git git-doc git-el") 
system("sudo apt-get install install openjdk-7-*")
system("sudo apt-get install libwxgtk-media3.0-dev tk-dev ")
system("sudo apt-get install git git-doc git-el")
system("sudo apt-get install python-gdal python3-gdal")
system("sudo apt-get install mpich2 cmake mesa-common-dev libglu1-mesa-dev bwidget")
system("sudo apt-get install libgmp3-dev libmpfr-dev libmpfr4 libgsl0-dev libgdal1-dev libproj-dev")
```
